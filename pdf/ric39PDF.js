import { obtenerRIC39 } from "./protocolosConsultas.js";
...
function generarInspecciones(datos) {
  ...
  const filas = [
    ["Aceptación según inspección visual", item.aceptacion_visual],
    ["Limpieza exterior", item.limpieza_exterior],
    ["Estado de baterías", item.estado_baterias],
...
function generarMediciones(datos) {
  const escenarios = ["NORMAL", "HIPERTENSO", "BRADICARDIA"];

  return escenarios.map(escenario => {
    const items = (datos.mediciones || []).filter(x => x.escenario === escenario);

    return `
      <div class="escenario">
        <div class="escenario-titulo">${esc(escenario)}</div>
...
export async function generarRIC39PDF(ric39_id) {
  const datos = await obtenerRIC39(ric39_id);
  ...
  const nombreArchivo = `RIC39_${descripcion}_${numeroSerie}_${ric39_id}_${fechaPDF}.pdf`;

  const variables = {
    CODIGO: "RIC39",
    TITULO: "VERIFICACIÓN DE MONITOR MULTIPARAMÉTRICO",
