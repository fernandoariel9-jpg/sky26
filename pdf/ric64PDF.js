import { obtenerRIC64 } from "./protocolosConsultas.js";
...
function generarInspecciones(datos) {
  ...
  const filas = [
    ["Inspección visual", item.inspeccion_visual],
    ["Limpieza exterior", item.limpieza_exterior],
    ["Limpieza interior", item.limpieza_interior]
  ];
  return `
    <table class="tabla">
...
function generarTemperaturas(datos) {
  const items = datos.temperaturas || [];
  return `
...
export async function generarRIC64PDF(ric64_id) {
  const datos = await obtenerRIC64(ric64_id);
  ...
  const nombreArchivo = `RIC64_${descripcion}_${numeroSerie}_${ric64_id}_${fechaPDF}.pdf`;

  const variables = {
    CODIGO: "RIC64",
    TITULO: "VERIFICACIÓN DE BAÑO TERMOSTÁTICO",
